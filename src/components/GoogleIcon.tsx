import React from 'react';
import { useSettings } from '../contexts/SettingsContext';

const useCurrentTheme = (overrideTheme?: 'Dark' | 'Light' | 'Neutral' | 'auto'): 'Dark' | 'Light' | 'Neutral' => {
  const { theme } = useSettings();
  if (overrideTheme && overrideTheme !== 'auto') {
    return overrideTheme;
  }
  return theme === 'light' ? 'Light' : 'Dark';
};

export interface GoogleIconProps {
  className?: string;
  alt?: string;
}

/**
 * Clean, crisp multi-colored Google "G" logo vector icon with zero corner noise or border artifacts.
 */
export const GoogleIcon: React.FC<GoogleIconProps> = ({
  className = "w-4 h-4 shrink-0",
  alt = "Google"
}) => {
  return (
    <svg
      viewBox="0 0 24 24"
      className={className}
      xmlns="http://www.w3.org/2000/svg"
      aria-label={alt}
      role="img"
    >
      <path
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
        fill="#4285F4"
      />
      <path
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
        fill="#34A853"
      />
      <path
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
        fill="#FBBC05"
      />
      <path
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
        fill="#EA4335"
      />
    </svg>
  );
};

export interface GoogleSignInButtonProps {
  onClick?: () => void;
  className?: string;
  theme?: 'Dark' | 'Light' | 'Neutral' | 'auto';
  shape?: 'Square' | 'Pill';
  showText?: boolean;
  disabled?: boolean;
  height?: number | string;
  ariaLabel?: string;
}

/**
 * Official Google Sign-In Button using Google Identity guidelines & assets from /public/google-login/
 * Automatically responds to current active light/dark theme.
 */
export const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  onClick,
  className = "",
  theme: themeProp,
  shape = "Square",
  showText = true,
  disabled = false,
  height = 40,
  ariaLabel = "Sign in with Google"
}) => {
  const activeTheme = useCurrentTheme(themeProp);
  const showTextStr = showText ? "yes" : "no";
  const assetPath = `/google-login/svg/${activeTheme.toLowerCase()}-${showTextStr}-${shape.toLowerCase()}.svg`;

  return (
    <button
      onClick={onClick}
      disabled={disabled}
      type="button"
      className={`inline-flex items-center justify-center p-0 border-none bg-transparent cursor-pointer transition-all hover:opacity-90 active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed ${className}`}
      aria-label={ariaLabel}
    >
      <img
        src={assetPath}
        alt={ariaLabel}
        style={{ height: typeof height === 'number' ? `${height}px` : height, width: 'auto' }}
        className="block max-w-full h-auto"
      />
    </button>
  );
};

export default GoogleIcon;
