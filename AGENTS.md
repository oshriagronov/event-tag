...
- **Admin & Management System:**
  - **AdminManagement Component & adminService:** Comprehensive 5-tool Admin suite (`/admin`) derived from Google Stitch MCP. Features dedicated side menu navigation for:
    ...
    5. **Quota & Tier Limits Management:** Form to edit Standard and Premium tier photo limits per 30-day rolling cycle (`maxPhotosPerMonth`), stored in `/system/config` and dynamically enforced at the Firestore Security Rules level (`getPhotoQuotaLimit()`).
  - **Quota & Tier Limits Enforcement (`quotaService`):** Client-side tier evaluation (`standard`, `premium`, `admin`) enforcing rolling 30-day photo limits (`maxPhotosPerMonth`), dynamic reset date calculation (`cycleReset` 30 days from first upload in cycle), proactive creation modal photo warnings/disabled states, and Firestore capacity/high-demand error interceptor (`isFirebaseQuotaOrDemandError`) with graceful bilingual notifications ("יש לנו עומס גבוה היום במערכת, אנא נסה שוב מחר.").
