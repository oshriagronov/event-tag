    ...
    <div className="max-w-6xl mx-auto px-4 py-8 flex-grow flex flex-col gap-8 transition-colors duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-200 dark:border-slate-800 pb-8">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-200 to-amber-400 dark:from-amber-505 dark:to-amber-700 flex items-center justify-center shadow-lg shadow-amber-500/20 dark:shadow-amber-600/30">
            <Sparkles className="w-6 h-6 text-amber-900 dark:text-white" />
          </div>
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight text-slate-805 dark:text-white m-0">EventTag</h1>
            <p className="text-slate-505 dark:text-slate-400 text-sm mt-0.5">לוח הבקרה שלי</p>
          </div>
        </div>
          ...
          {user && (
            <div className="flex items-center gap-3 bg-white/50 dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-2.5">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] text-slate-555">{user.email}</span>
              </div>
              <button
                onClick={signOut}
                className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-red-505 transition-all cursor-pointer"
                title="התנתק"
              >
      ...
      <div className="flex items-center justify-between">
        <div className="flex bg-white dark:bg-slate-955 p-1.5 rounded-xl border border-slate-200 dark:border-slate-850 gap-1 shadow-sm">
          <button
      ...
      {mode === 'cloud' && (
        <div className="flex flex-col gap-4">
          ...
          {loadingEvents ? (
            <div className="text-center py-12 text-slate-505 flex flex-col items-center gap-3">
              <Loader2 className="w-6 h-6 animate-spin text-amber-505" />
              <span>טוען אירועים...</span>
            </div>
          ) : cloudEvents.length === 0 ? (
            <div className="border border-dashed border-slate-350 dark:border-slate-850 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-4 bg-white/50 dark:bg-slate-900/20 shadow-sm">
              ...
              {cloudEvents.map((event) => {
                ...
                return (
                  <div
                    ...
                    className="group relative border border-slate-205 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-505/40 bg-white dark:bg-slate-900/60 rounded-2xl p-6 cursor-pointer transition-all duration-300 flex flex-col gap-5 shadow-sm hover:shadow-xl dark:shadow-none hover:-translate-y-1"
                  >
                    {/* Actions */}
                    <div className="absolute top-4 left-4 flex items-center gap-1.5 opacity-0 group-hover:opacity-100 transition-all z-10">
                      ...
                      <button
                        ...
                        title="מחק אירוע"
                        className="p-1.5 rounded-lg bg-white/80 dark:bg-slate-900/80 hover:bg-red-50 dark:hover:bg-red-555/20 border border-slate-205 dark:border-slate-800 hover:border-red-200 dark:hover:border-red-555/30 text-slate-400 dark:text-slate-555 hover:text-red-550 dark:hover:text-red-400 transition-all cursor-pointer shadow-sm"
                      >
                    ...
                    <div className="flex flex-col gap-1.5 pr-1">
                      ...
                      <div className="flex items-center gap-1.5 text-xs text-slate-505 mt-1">
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800/80 pt-4 mt-auto">
                      <div className="flex items-center gap-2">
                        <div className="p-2 rounded-lg bg-slate-50 dark:bg-slate-850/50 text-slate-400">
                          <ImageIcon className="w-4 h-4" />
                        </div>
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-555">תמונות</span>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-555">פנים מזוהות</span>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="px-4 py-2.5 rounded-xl bg-white dark:bg-slate-900 border border-slate-202 dark:border-slate-800 focus:border-amber-400 dark:focus:border-amber-505 focus:outline-none text-slate-800 dark:text-slate-100 text-sm w-64 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-colors shadow-sm"
              />
              <button
                onClick={async () => {
                ...
                className="px-5 py-2.5 rounded-xl bg-amber-100 hover:bg-amber-200 text-amber-900 dark:bg-amber-505 dark:hover:bg-amber-400 dark:text-slate-955 font-medium text-sm flex items-center gap-2 transition-all cursor-pointer shadow-md active:scale-95 border border-amber-200 dark:border-amber-600/30"
              >
          ...
          {!localEvents ? (
            ...
            <div className="border border-dashed border-slate-350 dark:border-slate-850 rounded-3xl p-12 text-center flex flex-col items-center justify-center gap-4 bg-white/50 dark:bg-slate-900/20 shadow-sm">
              ...
              <p className="text-slate-555 text-sm">אין אירועים מקומיים. צור אירוע חדש למעלה.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {localEvents.map((event) => {
                ...
                return (
                  <div
                    ...
                    className="group relative border border-slate-205 dark:border-slate-800 hover:border-amber-300 dark:hover:border-amber-505/40 bg-white dark:bg-slate-900/60 rounded-2xl p-6 cursor-pointer transition-all duration-300 flex flex-col gap-6 shadow-sm hover:shadow-xl hover:-translate-y-1"
                  >
                    <button
                      onClick={(e) => event.id !== undefined && handleDeleteLocalEvent(event.id, event.name, e)}
                      title="מחק אירוע"
                      className="absolute top-4 left-4 p-1.5 rounded-lg bg-white/80 dark:bg-slate-900/80 hover:bg-red-50 dark:hover:bg-red-555/20 border border-slate-202 dark:border-slate-800 hover:border-red-200 dark:hover:border-red-555/30 text-slate-400 dark:text-slate-505 hover:text-red-500 dark:hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all z-10 cursor-pointer shadow-sm"
                    >
                      <Trash2 className="w-4 h-4" />
                    </button>
                      ...
                      <div className="flex flex-col gap-2 mt-2 w-full text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-between text-xs gap-2">
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <span className="font-mono font-bold text-slate-755 dark:text-slate-350">
                              {scannedCount} / {totalToScan || event.photoCount || 0}
                            </span>
                          </div>
                        </div>
                        <div className="w-full bg-slate-150 dark:bg-slate-800 rounded-full h-2 overflow-hidden border border-slate-200 dark:border-slate-705">
                          <div
                            className="bg-amber-500 h-2 rounded-full transition-all duration-300 ease-out"
                            style={{
                              width: `${(totalToScan || event.photoCount) > 0 ? (scannedCount / (totalToScan || event.photoCount)) * 100 : 0}%`,
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-slate-100 dark:border-slate-800/80 pt-4 mt-auto">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-col">
                          <span className="text-xs text-slate-555">אורחים</span>
        ...
        <div className="fixed inset-0 bg-slate-955/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 max-w-md w-full shadow-2xl flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-slate-655 dark:text-slate-400">תיקיית Google Drive:</label>
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-805/50 rounded-xl px-4 py-3 border border-slate-200 dark:border-slate-800">
            ...
            <div className="flex flex-col gap-2">
              <label className="text-sm font-medium text-slate-655 dark:text-slate-400">שם האירוע:</label>
              <input
                type="text"
                value={newEventName}
                onChange={(e) => setNewEventName(e.target.value)}
                placeholder="למשל: חתונת יוסי ודנה 2026"
                className="px-4 py-3 rounded-xl bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 focus:border-amber-400 dark:focus:border-amber-505 focus:outline-none text-slate-800 dark:text-slate-100 text-sm placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-colors"
                autoFocus
              />
            </div>
            ...
            <div className="flex gap-3 mt-2">
              <button
                ...
                className="flex-1 py-3 rounded-xl bg-amber-505 hover:bg-amber-400 text-white dark:text-slate-955 font-bold text-sm transition-all cursor-pointer shadow-lg shadow-amber-505/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {creating ? (
      ...
      {/* QR Code Modal */}
      {qrEvent && (
        <div className="fixed inset-0 bg-slate-955/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setQrEvent(null)}>
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-3xl p-8 max-w-sm w-full shadow-2xl flex flex-col items-center gap-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center gap-2">
              <p className="text-xs text-slate-555 dark:text-slate-400 text-center">סרוק את הקוד כדי לפתוח את עמוד האירוע</p>
              <button
                onClick={(e) => handleCopyShareLink(qrEvent, e)}
                className="flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-100 dark:bg-amber-500/20 text-amber-900 dark:text-amber-400 text-sm font-bold border border-amber-200 dark:border-amber-505/30 hover:bg-amber-200 dark:hover:bg-amber-500/30 transition-all cursor-pointer"
              >
            ...
            <button
              ...
              className="text-slate-400 hover:text-slate-650 dark:hover:text-slate-200 text-sm transition-colors cursor-pointer"
            >
              סגור
            </button>
          </div>
