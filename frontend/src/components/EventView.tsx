  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 flex-grow flex flex-col gap-6 text-right transition-colors duration-300">
      ...
      {event.status === 'pending' && !isThisEventScanning && (
        <div className="border border-dashed border-slate-350 dark:border-slate-850 rounded-3xl p-16 text-center flex flex-col items-center justify-center gap-6 bg-white/50 dark:bg-slate-900/10 flex-grow py-24 shadow-sm">
      ...
      {(event.status === 'scanning' || isThisEventScanning) && (
        <div className="bg-white dark:bg-slate-900/40 border border-slate-205 dark:border-slate-800 rounded-3xl p-8 flex flex-col gap-6 shadow-xl max-w-2xl mx-auto w-full my-8">
              ...
              <button
                ...
                className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all cursor-pointer shadow-sm ${
                  isPaused
                    ...
                    : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-350 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700'
                }`}
              >
      ...
      {event.status === 'ready' && !isThisEventScanning && (
        <div className="flex flex-col gap-8">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="flex gap-2 mt-4">
                  <input
                    ...
                    readOnly
                    value={shareLink}
                    className="flex-grow px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 text-slate-650 dark:text-slate-400 text-sm font-mono focus:outline-none"
                  />
                  <button
                  ...
                  <button
                    ...
                    className="p-3 rounded-xl bg-white dark:bg-slate-900 border border-slate-250 dark:border-slate-800 text-slate-600 dark:text-slate-350 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors cursor-pointer"
                    title="הצג QR"
                  >
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-xl font-bold text-slate-805 dark:text-slate-100">גלריית תמונות אירוע</h3>
            {photos.length === 0 ? (
              <div className="text-center py-12 text-slate-505">טוען תמונות...</div>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                {photos.slice(0, 24).map((photo) => (
                  <div
                    key={photo.id}
                    className="relative aspect-square border border-slate-200 dark:border-slate-850 rounded-2xl overflow-hidden shadow-sm bg-slate-100 dark:bg-slate-900/50"
                  >
                    <CloudPhotoImage
